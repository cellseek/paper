\begin{tikzpicture}[
  ...
  % Memory reader
  \node[processor] (reader1) at (4,3.5) {Memory\\Reader};

  % Improved arrows from all memories - connecting to specific anchor points
  \draw[semithick] (mem1.east) -- (reader1.west);
  \draw[semithick] (mem2.east) -- (reader1.west);
  \draw[semithick] (mem3.east) -- (reader1.west);
  \draw[semithick] (mem4.east) -- (reader1.west);
  \draw[semithick] (mem5.east) -- (reader1.west);

  % Current frame
  \node[memory, fill=yellow!30] (current1) at (7.5,3.5) {Current\\Frame $t$};
  ...
  \node[processor, fill=orange!20] (reader2) at (4.5,-3) {Last-Frame\\Memory Reader};

  % Active arrow from only last frame
  \draw[semithick] (last_mem.east) -- (reader2.west);

  % Disabled arrows from old memories (optional - shows what's not used)
  \draw[semithick, dashed, color=red!50] (old_mem1.east) -- (reader2.west);
  \draw[semithick, dashed, color=red!50] (old_mem2.east) -- (reader2.west);
  \draw[semithick, dashed, color=red!50] (old_mem3.east) -- (reader2.west);
  \draw[semithick, dashed, color=red!50] (old_mem4.east) -- (reader2.west);

  % Current frame
  \node[memory, fill=yellow!30] (current2) at (8.5,-3) {Current\\Frame $t$};
